...
def _render_personagem_ativo(ativo, fichas):
    ...
    # cabeçalho compacto — rodada + init
    st.markdown(f"""
    ...
    if ficha_ativa:
        ...
        if "vida" not in status:
            status["vida"] = {"atual": ficha_ativa.get("hp_atual", 0),
                              "maximo": ficha_ativa.get("hp_max", 1)}
        ficha_ativa["status"] = status
        # bloco unificado — um único iframe com todas as barras
        ...
        <div class='hk-card' style='border-color:#5a4a30;'>
            <div style='font-family:Cinzel,serif;font-size:16px;
                        font-weight:700;letter-spacing:.2em;color:#c8b89a;'>
                {part.get('nome','?').upper()}
            </div>
...
def _render_iniciar_combate(fichas):
    ...
    with st.expander("⚔️ Configurar participantes"):
        ...
        for f in fichas:
            ...
            participantes.append({"nome": f["nome"], "iniciativa": init,
                                  "tipo": "jogador", "ficha_id": f["id"]})
    ...
    for i, part in enumerate(participantes):
        ...
        with cols[i]:
            ...
            if ficha:
                ...
                st.markdown(f"""
                <div style='font-size:11px;color:#6a5a40;text-align:center;padding:4px;'>
                    HP {hp} / {hp_max}
                </div>
                """, unsafe_allow_html=True)
